...
static inline bool handle_api_install_direct(sb_Stream *s, const json_t *root)
{
    ...
    if (!pkg_setup_prerequisites(
            piece_urls, piece_count, ref_pkg_json_path, param_sfo_path, icon0_png_path, &content_type, &package_size, &is_patch, &has_icon, error_buf, sizeof(error_buf)))
    {
        rtrim(error_buf);
        if (*error_buf != '\0')
    ...
    if (bgft_download_register_package_task(
            content_id, content_url, title_name, has_icon ? icon_path : NULL, package_type, package_sub_type, package_size, is_patch, &task_id, &ret))
    {
...
static bool handle_api_get_task_progress(sb_Stream *s, const char *method, const char *path, char *in_data, size_t in_size)
{
    ...
    if (bgft_download_get_task_progress(task_id, &progress_info, &ret))
    {
        ...
        sb_writef(s,
                  "{ \"status\": \"success\", \"bits\": 0x%" PRIX32 ", \"error\": %d, \"length\": 0x%" PRIXMAX ", \"transferred\": 0x%" PRIXMAX ", \"length_total\": 0x%" PRIXMAX
                  ", \"transferred_total\": 0x%" PRIXMAX ", \"num_index\": %" PRIu32 ", \"num_total\": %" PRIu32 ", \"rest_sec\": %" PRIu32 ", \"rest_sec_total\": %" PRIu32
                  ", \"preparing_percent\": %" PRId32 ", \"local_copy_percent\": %" PRId32 " }\n",
                  progress_info.bits,
                  progress_info.error_result,
                  (uintmax_t)progress_info.length,
                  (uintmax_t)progress_info.transferred,
                  (uintmax_t)progress_info.length_total,
                  (uintmax_t)progress_info.transferred_total,
                  progress_info.num_index,
                  progress_info.num_total,
                  progress_info.rest_sec,
                  progress_info.rest_sec_total,
                  progress_info.preparing_percent,
                  progress_info.local_copy_percent);
    }
    else
    {
        kick_error_json(s, ret);
