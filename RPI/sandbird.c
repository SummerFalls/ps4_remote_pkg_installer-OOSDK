...
const char *sb_error_str(int code)
{
    switch (code)
    {
        ...
        case SB_EBADSTATE:
            return "bad stream state for this operation";
        case SB_EBADRESULT:
            return "bad result code from event handler";
        case SB_ECANTOPEN:
...
static int sb_stream_emit(sb_Stream *st, sb_Event *e)
{
    ...
    {
        case SB_RES_CLOSE:
            sb_stream_close(st); /* Fall through */
        case SB_RES_OK:
            return SB_ESUCCESS;
        default:
            return SB_EBADRESULT;
    }
}
...
close_stream:
            ...
            if (st->state == STATE_RECEIVING_HEADER && st->recv_buf.len >= 4 && mem_equal(st->recv_buf.s + st->recv_buf.len - 4, "\r\n\r\n", 4))
            {
                const char *s;
...
sb_Server *sb_new_server(const sb_Options *opt)
{
    ...
    if ((tmp_end == opt->port) || (errno == ERANGE) || !(port > 0 && port <= USHRT_MAX))
    {
        goto fail;
    }
